 ...
 * 
 */
public class ModifyThreeTestBean implements Serializable {
	/**
	 * 
	 */
		...
		try {
			...
			vitalSignsValues2 = getVitalSignsValues2() == null ? "thisIsNull"
					: java.net.URLEncoder.encode(getVitalSignsValues2(),
							"utf-8");
			orgVsId = getOrgVsId() == null ? "thisIsNull" : java.net.URLEncoder
					.encode(getOrgVsId(), "utf-8");
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
		}
		return "?recordingTime=" + recordingTime + "&patId=" + patId
				+ "&vitalSigns=" + vitalSigns + "&vitalSignsValues="
				+ vitalSignsValues + "&logBy=" + logBy + "&vitalSignsValues2="
				+ vitalSignsValues2 + "&orgVsId=" + orgVsId;
	}
	...
	public String getOrgVsId() {
		return orgVsId;
	}

	public void setOrgVsId(String orgVsId) {
		this.orgVsId = orgVsId;
	}
